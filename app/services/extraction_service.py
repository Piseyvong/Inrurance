...
from app.services.policy_service import get_policy_for_claim
...
async def run_extraction_for_document(document_id: int, db: AsyncSession) -> list[ExtractedField]:
    ...
    try:
        # One OCR run has one current semantic extraction snapshot. Re-running
        # extraction cannot leave stale universal-schema fields beside it.
        await db.execute(delete(ExtractedField).where(ExtractedField.ocr_run_id == ocr_run.id))
        for value in extraction.values:
            field = ExtractedField(
                ...
                original_ocr_value=value.original_ocr_value,
            ...
            if value.field_name in {"invoice_number", "receipt_number", "medical_report_number", "report_number", "estimate_number"} and value.normalized_value:
                document.reference_number = value.normalized_value.strip()
        db.add(
            AuditLog(
