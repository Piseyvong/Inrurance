...
export function CustomerPortalPage() {
  ...
  return <div className="pageStack portalPage"><header className="portalHero"><div><p className="landingEyebrow">Customer portal</p><h1>Hello, {data.user.full_name}.</h1><p>Your policies and claims are securely scoped to this account.</p></div></header>{error ? <Alert tone="danger">{error}</Alert> : null}<section className="portalGrid"><article className="panel"><h2>My Policies</h2>{data.policies.map((policy: any) => <Link className="portalRow" to={`/customer/policies/${policy.id}`} key={policy.id}><div><strong>{policy.product.name}</strong><span>{policy.policy_number} · {policy.start_date} to {policy.end_date}</span></div><StatusBadge status={policy.status}/></Link>)}</article><article className="panel"><div className="sectionHeading"><h2>My Claims</h2>{data.claims.length ? <Link className="tableAction" to="/customer/claims">View all<ArrowRightIcon size={14}/></Link> : null}</div>{data.claims.length ? data.claims.slice(0, RECENT_CLAIMS_LIMIT).map((claim: any) => <Link className="portalRow" key={claim.id} to={`/claims/${claim.id}/documents`}><div><strong>{claim.claim_number}</strong><span>{claim.type} - {claim.amount ? `${claim.amount} ${claim.currency}` : "Awaiting document extraction"}</span></div><StatusBadge status={claim.status}/></Link>) : <p>No claims yet.</p>}</article></section><section className="panel"><div className="sectionHeading"><div><h2>Start a Claim</h2><p>Select an owned policy, then upload the required documents. Document AI extracts the claim details and prepares the summary after review.</p></div><button type="submit" form="startClaimForm" disabled={!data.policies.length}>Continue to documents</button></div><form id="startClaimForm" className="portalClaimForm" onSubmit={submit}><label>Policy<select value={selected} onChange={(event) => setSelected(event.target.value)}>{data.policies.map((policy: any) => <option value={policy.id} key={policy.id}>{policy.product.name} - {policy.policy_number}</option>)}</select></label></form></section></div>;
}
